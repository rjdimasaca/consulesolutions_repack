...
define(['N/search'], function (search) {
    ...
    function buildHtml(opts) {
        ...
        return ''
            ...
            + '  var HAS_MAX_QTY = (MAX_QTY_RAW !== null && MAX_QTY_RAW !== undefined && String(MAX_QTY_RAW).trim() !== "");'
            + '  var MAX_QTY = parseFloat(MAX_QTY_RAW); if (isNaN(MAX_QTY)) MAX_QTY = 0;'
            + '  var ALLOW_LOTNUMBER_DEFAULT_QTY = true;'
            ...
            + '    if (cb.checked){'
            + '      if (ALLOW_LOTNUMBER_DEFAULT_QTY){'
            + '        var cur = toNum(qtyEl.value);'
            + '        if (!cur){'
            + '          var defVal = avail;'
            + '          if (HAS_MAX_QTY){ defVal = Math.min(defVal, MAX_QTY); }'
            + '          qtyEl.value = (defVal || "");'
            + '        }'
            + '      }'
            + '    }'
            ...
            + '      total += qn;'
            + '    }'
            + '    if (HAS_MAX_QTY && total > MAX_QTY + 1e-9){'
